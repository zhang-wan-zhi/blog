    ...
    if (docs.length > 0) {
      if (req.body.avatar == "null" || req.body.avatar.indexOf("avatar") > -1) {
        ...
        let saveImg = path.join(
          __dirname,
          "../public/static/upload/avatar/" + D + ".png" // 写入的地址
        ); //api.js的上级的status下
...
// 文章图片保存
router.post("/api/save/img", (req, res) => {
  const fs = require("fs");
  let D = Date.now();
  let saveImg = path.join(
    __dirname,
    "../public/static/upload/img/" + D + ".png" // 写入的地址
  );
  let pathImg = "./static/upload/img/" + D + ".png"; //返前台路径目录
  let base64 = req.body.avatar.replace(/^data:image\/\w+;base64,/, "");
  let dataBuffer = new Buffer(base64, "base64"); //把base64码转成buffer对象，
  fs.writeFile(saveImg, dataBuffer, function(err) {
    //用fs写入文件
...
// 删除图片
router.post("/api/delete/img", (req, res) => {
  const fs = require("fs");
  const url = "../public/" + req.body.avatar;
...
router.post("/api/messagelist", function(req, res) {
  ...
  var query = db.Messages.find({});
  query.skip((page - 1) * rows); // 跳过
  query.limit(rows); // 查询
  console.log('123');
  //计算分页数据
  query.exec(function(err, rs) {
    ...
    if (err) {
      ...
      //计算数据总数
      if(rs <= 0) {
        res.send({status:"0",msg:"没有数据了"})
        return
      }
      console.log('成功找到数据',rs);
      let data_msg = [];
...
// 留言保存
router.post("/api/message/save", (req, res) => {
  let newMessages = new db.Messages(req.body.messagesForm);
  newMessages.save(function(err) {
    if (err) {
      res.send(err);
    } else {
      res.send({ status: 1, msg: "留言成功" });
    }
  });
});
// 文章更新
